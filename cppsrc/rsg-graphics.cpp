...
{
	rsd::uint2* display_extents = parent_window->GetDisplaySize();
	if ((origin + size) > *display_extents ||
		(titletext.length() + 2) > size.x + 6) {
		return false;
	}

	Uint32 start_index = parent_window->PointToIndex(origin);
	...
	//draw title bar bg
	parent_window->FillCharacter(start_index, size.x, title_char);
		...
		++bgline) 
	{
		...
		next_origin += origin;
		Uint32 line_index = parent_window->PointToIndex(next_origin);
		/*SDL_Log("Line at (%i, %i) = %i",
			next_origin.x, next_origin.y, line_index);*/
		bg_char->char1 = ' ';
		parent_window->FillCharacter(line_index + 1, size.x - 2, bg_char);
		bg_char->char1 = 257;
		parent_window->SetCharacter(line_index, bg_char);
		parent_window->SetCharacter(line_index + size.x - 1, bg_char);
	}
